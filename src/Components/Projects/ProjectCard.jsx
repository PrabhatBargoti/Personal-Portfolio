...
const ProjectCard = ({ number, title, image, techStack, link }) => {
  return (
    ...
    >
      ...
      <span className='relative z-10 font-[FiraCode] text-acid text-2xl font-bold mr-6 md:mr-10 shrink-0'>
        {number}
      </span>
      <div className='relative z-10 flex flex-col gap-3'>
        <h3 className='font-[Syne] text-4xl md:text-6xl font-bold uppercase text-white leading-none transition-colors duration-300'>
          {title}
        </h3>
        <div className='flex items-center gap-2'>
          {techStack.map((tech, i) => (
            <span
              key={i}
              className='font-[FiraCode] text-[11px] uppercase tracking-wide text-[#CAC2BD] border border-hairline-border rounded-sm px-2 py-1'
            >
              {tech}
            </span>
          ))}
      ...
      <span className='relative z-10 ml-auto hidden md:flex items-center gap-1 font-[FiraCode] text-[12px] uppercase tracking-widest text-acid shrink-0'>
        Explore More
